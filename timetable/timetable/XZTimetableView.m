...
-(void)loadViews{
    [self drawBorderInView:_mainScrollView withBorderWidth:1.0];
...
{   CGFloat cellWidth = scrollView.frame.size.width/(numberOfColumns);
    ...
    for (NSInteger i=0; i < numberOfColumns; i++) {
        for (NSInteger j=0; j < numberOfRows; j++) {
            ...
            if ([_dataSourse timetableView:self cellInPositon:position]!=nil) {
                XZCell *cell;
                cell = [_dataSourse timetableView:self
...
-(void)drawBorderInView:(UIScrollView *)sc withBorderWidth:(CGFloat)borderWidth{
    for (int i = 0; i <= 12; i++) {
        UIView *view = [[UIView alloc]initWithFrame:CGRectMake(0, i*sc.frame.size.height/8.0, sc.frame.size.width,borderWidth )];
        view.backgroundColor = [UIColor whiteColor];
        [sc addSubview:view];
    }
    for (int j = 0; j <= 7; j++) {
        UIView *view = [[UIView alloc]initWithFrame:CGRectMake(j*sc.frame.size.width/7.0, 0, borderWidth,sc.frame.size.height/8.0*12)];
        view.backgroundColor = [UIColor whiteColor];
        [sc addSubview:view];
    }
}
