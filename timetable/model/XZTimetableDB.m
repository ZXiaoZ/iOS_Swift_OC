...
-(void)initDataBase{
    ...
    if (![_fmDB tableExists:@"timetable"]) {
        [_fmDB executeUpdate:@"create table timetable (id integer primary key autoincrement not null,courseName text,teacherName text,classroom text,schooltime text,weeks text,startingAndEndingTime text,examination text,checkingStandard text,maxAbsenceNumber text,homework text)"];
    }
...
-(void)insertTimetable:(NSDictionary *)timetable{
    ...
    NSMutableDictionary *mDic = [NSMutableDictionary dictionaryWithDictionary:timetable];
    [mDic removeObjectForKey:@"ID"];
    timetable = (NSDictionary *)mDic;
...
-(void)insertNote:(NSDictionary *)note{
    ...
    NSMutableDictionary *mDic = [NSMutableDictionary dictionaryWithDictionary:note];
    [mDic removeObjectForKey:@"ID"];
    note = (NSDictionary *)mDic;
...
-(void)insertAttendance:(NSDictionary *)attendance{
    ...
    NSMutableDictionary *mDic = [NSMutableDictionary dictionaryWithDictionary:attendance];
    [mDic removeObjectForKey:@"ID"];
    attendance = (NSDictionary *)mDic;
...
//跟新数据
-(void)updateTable:(NSString *)updateSQL{
    if (![_fmDB open]) {
        NSLog(@"Open database failed");
        return;
    }
    BOOL update = [_fmDB executeUpdate:updateSQL];
    if (!update) {
        NSLog(@"更新数据失败");
    }
    [_fmDB close];
